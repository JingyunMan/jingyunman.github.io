...
var ReactionWorld={
    init: function(){
        ...
        if(text.diffusionLiquid){
            ...
            this.solvent(particleSystem, 0, 1.8, 1, 1, 117, 107, 177);
        }
         //液体固体 Solid dissolving
        if(text.dissolveSolid){
            this.solvent(particleSystem, 0, 6, 4.5, 3, 198, 219, 239);                 
            this.solid_particles(particleSystem, 1, new b2Vec2(0, 1.8), new b2ParticleColor(117, 107, 177));
        }
        //LLL
        if(text.LLL){
            this.solvent(particleSystem, 0, 4, 8, 1.5, 198, 219, 239);
            this.solvent(particleSystem, 0, 1.5, 2, 1, 117, 107, 177);
        }
        //LLS
        if(text.LLS){
            this.solvent(particleSystem, 0, 3.5, 8, 1.6, 198, 219, 239);
            this.solvent(particleSystem, 0, 1.2, 1, 0.5, 117, 107, 177);
        }
        //LLG
        if(text.LLG){
            this.solvent(particleSystem, 0, 3.5, 8, 1.6, 198, 219, 239);
        ...
        //LGS
        if(text.LGS){
            ReactionWorld.solvent(particleSystem, 0, 2.6, 7.5, 2, 217, 217, 217); 
        }
       
        //heating
        if(text.heater){
            this.solvent(particleSystem, 0, 3, 8, 2, 198, 219, 239);
        }
        //水
        // if(text.phase){
        //     // this.H2O(-1, 7, 0);
...
var renderer = {
    ...
    drawParicles: function(system){
        ...
        // var parRadius=system.radius*windowWidth*0.04;
        var scale=(windowWidth<windowHeight ? windowWidth : windowHeight)*0.045;
        var parRadius=system.radius*scale;
        if(text.dissolveSolid)
